...
const ProductPage = () => {
    ...
    <div className="min-h-screen   flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 bg-white rounded-lg  p-6 lg:p-8">
                    <div className="space-y-4">
                        <div className="aspect-square w-full bg-gray-100 rounded-lg overflow-hidden">
                            <Image   src={  apiProduct[0]?.images[0]?.src || "/image1.jpg" } alt={"products image"}  priority width={200} height={0} className="w-full h-full object-cover"/>
                        </div>

                        <div className="grid grid-cols-4 gap-3">
                            {apiProduct[0]?.images.map((image, index) => (
                                <button key={index} onClick={() => setSelectedImage(index)}
                                    className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${
                                        selectedImage === index ? 'border-blue-600 shadow-md': 'border-gray-200 hover:border-gray-300' }`}>
                                    <img  src={image.src}  alt={`${product.name} ${index + 1}`} className="w-full h-full object-cover"/>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col space-y-6">
                        <div className="pb-6 border-b-2 border-gray-200">
                            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-3">
                                {apiProduct[0]?.categories[0].name}
                            </h1>
                            <p className="text-3xl font-semibold text-blue-600">
                                RS {apiProduct[0]?.price}
                            </p>
                        </div>

                        <div>
                            <p className="text-gray-700 leading-relaxed" ><span dangerouslySetInnerHTML={{ __html: apiProduct[0]?.short_description }} /></p>
                        </div>

                        {apiProduct[0]?.featured && (
                        ...
                        <button  onClick={() => setIsWishlisted(!isWishlisted)}
                            className={`flex cursor-pointer  items-center justify-center gap-2 px-6 py-3 rounded-lg border-2 transition-all ${
                        ...
                        <div className="space-y-3 pt-4 border-t border-gray-200">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {  apiProduct[0]?.categories.map((category, index) => (
                                    <span  key={index} className="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded-full hover:bg-gray-200 cursor-pointer transition-colors">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                               ...
                               {  apiProduct[0]?.tags.map((tags, index) => (
                                    <span  key={index} className="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded-full hover:bg-gray-200 cursor-pointer transition-colors">
                                        {tags?.name || "undefined"} 
                                    </span>
                                ))  }
            ...
            <div className="max-w-7xl my-8 mx-auto px-4  sm:px-6 lg:px-8 py-8 bg-white text-center">
                ...
                <div>
                    <p className="text-gray-700 leading-relaxed" ><span dangerouslySetInnerHTML={{ __html: apiProduct[0]?.description }} /></p>
                </div>
                </div>
            </div>
        </div>
