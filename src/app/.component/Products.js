...
const Products = () => {
    ...
    const [toatalProducts , setTotalProducts] = useState("");
  ...
  <div className='flex flex-col' >
        ...
        <div className='my-1 py-1 grid grid-cols-2 items-center justify-evenly ' >
            <p className='text-gray-800 text-[15px] ml-9' >Showing 1-{showProduct} of {toatalProducts} results</p>
                ...
                {products.map( value =>(
                  <div key={value.id} className={`${changeDiv ? "grid grid-cols-[35%_auto] gap-4 my-3  h-[300px]" :"h-[400px] w-[300px] "} group  relative overflow-hidden rounded-md cursor-pointer  transition-all duration-300`}>
                    <div className={`${changeDiv? "flex  items-center justify-center" :"relative"}h-[70%]   w-full overflow-hidden`}>
                      ...
                      <Image onClick={()=>{router.push(`/shop/${value.slug}`)}} unoptimized  src={value.images[0]?.src || "/image1.jpg" } alt={value.images[0]?.alt || "products image"}  priority  width={256} height={0}
                    ...
                    <div className="px-2 py-4 ">
                      <h3 className=" font-semibold text-gray-700 leading-nomal hover:text-blue-500">{value.name || "Products"}</h3>
